...
-- the answer back.
--
-- Deliberately NOT the `jobs` table. Nothing here touches a phone, a pool
-- or the builder, and a checker sharing the build queue would stop
-- answering the moment somebody paused building - two unrelated jobs held
-- hostage by one switch (2026-09-24).
--
-- `run_id` groups one paste, so the page can show "this run" without
-- guessing from timestamps. `from_cache` says the answer was copied from
-- a recent check rather than asked again: customers send the same
-- addresses twice, and the exits are the scarce thing.
CREATE SEQUENCE IF NOT EXISTS origin_run_id;
...
CREATE TABLE IF NOT EXISTS origin_checks (
    ...
    -- Empty until it is settled. `unclear` is a real answer and kept as
    -- one: it is what a retry is offered on.
    verdict     text NOT NULL DEFAULT ''
                CHECK (verdict IN ('', 'google', 'password', 'none',
                                   'unclear')),
    reason      text NOT NULL DEFAULT '',
    exit_name   text NOT NULL DEFAULT '',
    -- Whether CapSolver had to clear a Cloudflare challenge for it, so
    -- what the solver costs can be read off the table.
...
CREATE INDEX IF NOT EXISTS origin_checks_queued
    ON origin_checks (status, id);
CREATE INDEX IF NOT EXISTS origin_checks_run
    ON origin_checks (run_id, id);
-- What the cache asks: the newest settled answer for this address. Only
-- the three that are answers - an `unclear` teaches nothing and must not
-- be served to the next person who asks.
CREATE INDEX IF NOT EXISTS origin_checks_recent
    ON origin_checks (lower(address), settled_at DESC)
    WHERE status = 'done' AND verdict IN ('google', 'password', 'none');
