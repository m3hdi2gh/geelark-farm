...
def test_the_scan_sees_the_reasons_the_router_builds_from_a_screen_name():
    ...
    # Nineteen distinct across the flows, not twenty-three: the two login flows
    ...
    # became a screen that answers the code OpenAI emails. The nineteenth is
    # `recovery_email_confirm`, which was a fatal page in the same way until it
    ...
    assert len({r for r in reported if r.startswith("stuck_on_")}) == 30
    # Thirty since 2026-09-17: the Spotify login adds code_page,
    # connection_error and logged_out_home.
