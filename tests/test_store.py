...
# ------------------------------------- the origin checker's queue (rev 36)
def test_a_paste_is_one_run_and_a_known_address_costs_no_exit(monkeypatch,
                                                              make_settings):
    """Customers send the same addresses again and a usable exit is the
    scarce thing, so an address answered this week is copied into the new
    run at once rather than asked about a second time (2026-09-24)."""
    from geelark_farm.store import origins

    # nextval, then the cache read, then one INSERT per address.
    conn = _ScriptedConn([(71,),
                          [("old@x.com", "google", "handed over to Google")],
                          None, None, None])
    monkeypatch.setattr(origins, "connect", lambda s: conn)

    got = origins.enqueue(make_settings(),
                          ["New@x.com", "old@x.com", "new@x.com", "  "],
                          asked_by=3)

    assert got == {"run_id": 71, "queued": 1, "cached": 1,
                   "addresses": ["New@x.com", "old@x.com"]}, (
        "the same address twice is one question, and blanks are nothing")
    cached = [q for q in conn.sql if "'done'" in q and "from_cache" in q]
    assert len(cached) == 1 and "true" in cached[0]
    # Only real answers are served from the cache: an `unclear` is one bad
    # minute on one exit, not a verdict about the address.
    reading = [q for q in conn.sql if "DISTINCT ON" in q][0]
    assert "verdict = ANY(%s)" in reading
    assert conn.params[1][0] == list(origins.KEEPABLE)
    assert "unclear" not in origins.KEEPABLE
    assert conn.committed == 1


def test_asking_again_is_allowed_by_turning_the_cache_off(monkeypatch,
                                                          make_settings):
    from geelark_farm.store import origins

    conn = _ScriptedConn([(72,), None])
    monkeypatch.setattr(origins, "connect", lambda s: conn)

    got = origins.enqueue(make_settings(), ["a@x.com"], cache_days=0)

    assert got["queued"] == 1 and got["cached"] == 0
    assert not [q for q in conn.sql if "DISTINCT ON" in q], (
        "no cache was read at all")


def test_two_checkers_never_take_one_row(monkeypatch, make_settings):
    """The same guarantee the build queue rests on, for the same reason:
    a row handed out twice is an exit spent twice on one question."""
    from geelark_farm.store import origins

    conn = _ScriptedConn([[(9, "a@x.com", 71, "running", "", "", "", False,
                            False, None, None, 0.0)], None])
    monkeypatch.setattr(origins, "connect", lambda s: conn)

    got = origins.take(make_settings(), "checker-1", limit=1)

    assert got[0]["id"] == 9 and got[0]["address"] == "a@x.com"
    taking = [q for q in conn.sql if "SKIP LOCKED" in q]
    assert taking and "status = 'queued'" in taking[0]
    assert "ORDER BY id" in taking[0], "oldest first, so a run fills in order"
    assert origins.take(make_settings(), "checker-1", limit=0) == []


def test_a_word_the_checker_invents_is_written_unclear_not_refused(
        monkeypatch, make_settings):
    """A row must always be closable. If `settle` refused an unknown
    word, a checker that learned a new screen would leave the row
    `running` for ever and the page would wait on it."""
    from geelark_farm.store import origins

    conn = _ScriptedConn([None])
    monkeypatch.setattr(origins, "connect", lambda s: conn)

    origins.settle(make_settings(), 9, "apple", "handed over to Apple",
                   exit_name="SX43", cleared=True, seconds=21.4)

    params = conn.params[0]
    assert params[0] == "unclear"
    assert "'apple'" in params[1], "and it says what was actually said"
    assert params[2] == "SX43" and params[3] is True
    assert conn.committed == 1


def test_a_retry_reopens_the_same_row(monkeypatch, make_settings):
    """A second row would double the run's list and its counts; the
    operator is asking the same question again, not a new one."""
    from geelark_farm.store import origins

    conn = _ScriptedConn([None], rowcounts=[2])
    monkeypatch.setattr(origins, "connect", lambda s: conn)

    assert origins.retry(make_settings(), 71) == 2
    sql = conn.sql[0]
    assert "status = 'queued'" in sql and "verdict = 'unclear'" in sql
    assert "settled_at = NULL" in sql and "from_cache = false" in sql


def test_the_run_reads_its_rows_and_its_tally(monkeypatch, make_settings):
    from geelark_farm.store import origins

    conn = _ScriptedConn([[(9, "a@x.com", 71, "done", "google", "", "SX43",
                            False, False, None, None, 12.0)],
                          (12, 5, 4, 1, 1, 1), (71,)])
    monkeypatch.setattr(origins, "connect", lambda s: conn)
    s = make_settings()

    rows = origins.run(s, 71)
    assert rows[0]["verdict"] == "google" and rows[0]["exit_name"] == "SX43"
    assert origins.counts(s, 71) == {"all": 12, "google": 5, "password": 4,
                                     "none": 1, "unclear": 1, "working": 1}
    assert origins.latest_run(s) == 71
    # A read is a read: nothing here commits.
    assert conn.committed == 0 and conn.rolled_back == 3


def test_other_peoples_addresses_are_not_kept_for_ever(monkeypatch,
                                                       make_settings):
    from geelark_farm.store import origins

    conn = _ScriptedConn([None], rowcounts=[40])
    monkeypatch.setattr(origins, "connect", lambda s: conn)

    assert origins.purge(make_settings(), keep_days=90) == 40
    assert "DELETE FROM origin_checks" in conn.sql[0]
    assert conn.params[0] == (90,)


def test_the_origin_tables_shape_says_what_it_means():
    """The checks table is its own, not the build queue's: a paused farm
    must not stop answering a question that touches no phone."""
    from geelark_farm.store import db as store_db

    sql = store_db.schema_sql()
    assert "CREATE TABLE IF NOT EXISTS origin_checks" in sql
    assert "CREATE SEQUENCE IF NOT EXISTS origin_run_id" in sql
    for word in ("'google'", "'password'", "'none'", "'unclear'"):
        assert word in sql, word
    assert "origin_checks_recent" in sql, "the cache has an index"
    assert store_db.SCHEMA_REV == "36"
